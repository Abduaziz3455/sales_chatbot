...
room_prompt = PromptTemplate(input_variables=["question"], template="""As an expert in text extraction, your task is to extract room numbers from user questions. 
    ...
    If the user's question contains a room number, return the number. For example, if the user's question is 'ikki xonali uylar narxi qancha?' 
...
class FlatRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(self, query_dict: dict, *, run_manager: CallbackManagerForRetrieverRun) -> List[
        Document]:
